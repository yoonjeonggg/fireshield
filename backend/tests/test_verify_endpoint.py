...
# 매번 danger 등급이 나오도록 구성한다 (반복 신고 누적을 위해 등급을 고정시켜 둠).
_FRAUD_CLAIM_TYPE = "긴급 영업정지 대상이니 오늘까지 계좌로 즉시 입금하세요"


def _fraud_payload(account_number, target_business="존재하지않는가짜소방업체"):
    return {
        "claimed_org": "서울소방서",
        "claimed_person": "김소방",
        "claim_type": _FRAUD_CLAIM_TYPE,
        "target_business": target_business,
        "account_number": account_number,
    }


def test_verify_without_account_number_has_no_fraud_evidence(monkeypatch):
    monkeypatch.setattr(ai_verify.settings, "ai_enabled", False)
    resp = client.post("/api/v1/verify", json=_fraud_payload(None))
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_verify_with_unreported_account_shows_no_history(monkeypatch):
    monkeypatch.setattr(ai_verify.settings, "ai_enabled", False)
    resp = client.post("/api/v1/verify", json=_fraud_payload("110-999-000001"))
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_verify_flags_repeatedly_reported_account(monkeypatch):
    monkeypatch.setattr(ai_verify.settings, "ai_enabled", False)
    account_number = "110-999-000002"

    # 임계치만큼 같은 계좌로 위험 판정을 쌓는다 (_FRAUD_CLAIM_TYPE이라 매번 danger).
    for _ in range(settings.fraud_check_report_threshold):
        prior = client.post("/api/v1/verify", json=_fraud_payload(account_number))
        assert prior.json()["risk_level"] == "danger"

    # 표기 방식(하이픈 위치)이 달라도 같은 계좌로 인식되어야 한다.
    reformatted_account_number = account_number.replace("-", "")
    resp = client.post("/api/v1/verify", json=_fraud_payload(reformatted_account_number))
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_verify_different_account_number_is_unaffected(monkeypatch):
    monkeypatch.setattr(ai_verify.settings, "ai_enabled", False)
    reported_account = "110-999-000003"
    other_account = "220-111-222222"

    for _ in range(settings.fraud_check_report_threshold):
        client.post("/api/v1/verify", json=_fraud_payload(reported_account))

    resp = client.post("/api/v1/verify", json=_fraud_payload(other_account))
    body = resp.json()
